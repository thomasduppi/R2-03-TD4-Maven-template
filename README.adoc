...
. Reprenez un de vos exercice précédent et mettez les sources dans `src/java`.
. Ajustez éventuellement le `pom.xml` pour que les étapes précédentes produisent les résultats escomptés.
. Ajoutez les commentaires vus en dev.
. Cherchez dans la doc {maven} ce qu'il vous faut ajouter à votre `pom.xml` pour que {maven} génère la documentation automatiquement pour vous.
